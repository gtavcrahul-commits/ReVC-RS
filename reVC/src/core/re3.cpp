...
#if defined ANDROID
/* ------------------------------------------------------------------
 * File logging support (Android).
 * Mirrors every debug / trace / usererror message into a log.txt file
 * inside the game's external storage dir, since logcat is unreliable
 * on some devices.
 * ------------------------------------------------------------------ */
static FILE *g_re3_logfile = NULL;
static bool g_re3_logfile_tried = false;

static void re3_logfile_write(const char *tag, const char *msg)
{
    if (!g_re3_logfile_tried) {
        g_re3_logfile_tried = true;
        const char *root = getenv("STORAGE_ROOT");
        if (root != NULL && root[0] != '\0') {
            char path[1024];
            /* Avoid double slash if STORAGE_ROOT already ends with one. */
            size_t rl = strlen(root);
            bool hasSlash = (rl > 0 && root[rl - 1] == '/');
            snprintf(path, sizeof(path), "%s%slog.txt",
                     root, hasSlash ? "" : "/");
            /* Truncate on first call so each run starts fresh. */
            g_re3_logfile = fopen(path, "w");
            if (g_re3_logfile != NULL) {
                fprintf(g_re3_logfile,
                        "=== reVC log started; STORAGE_ROOT=%s ===\n", root);
                fflush(g_re3_logfile);
            }
        }
    }
    if (g_re3_logfile != NULL) {
        if (tag != NULL) fprintf(g_re3_logfile, "[%s] ", tag);
        fputs(msg, g_re3_logfile);
        size_t n = strlen(msg);
        if (n == 0 || msg[n - 1] != '\n') fputc('\n', g_re3_logfile);
        /* Flush every line so a crash doesn't lose recent output. */
        fflush(g_re3_logfile);
    }
}
#endif
...
void re3_debug(const char *format, ...)
{
    ...
    re3_logfile_write("DBG", re3_buff);
#endif
        printf("%s", re3_buff);
...
void re3_usererror(const char *format, ...)
{
    ...
    re3_logfile_write("ERROR", re3_buff);
    va_end(va);
    /* Don't abort on Android: usually triggered by missing GTA VC asset
     * files. Logging instead lets the game continue trying so the user
     * can see in logcat (or log.txt) exactly which files are missing. */
    return;
#endif
        printf("\nreVC Error!\n\t%s\n",re3_buff);
        assert(false);
