...
{
#if defined(ANDROID)
        /* Audio fully disabled on Android until game data files are present.
         * Initialising AudioManager / cSampleManager without the original
         * GTA VC `audio/` folder triggers fatal asserts (and the asset files
         * are not shipped with this engine). Skip silently so the game can
         * proceed to render its first frame. */
        return;
#else
